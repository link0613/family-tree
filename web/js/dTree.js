  ...
  var TreeBuilder = (function () {
    ...
    _createClass(TreeBuilder, [{
      ...
      value: function _update(source) {
        ...
        /*
         this.svg.selectAll('.sibling').data(this.siblings).enter().append('path')
            .attr('class', opts.styles.marriage).each(function () {
            // Generate random color for each marriage line
            var randomColor = 'stroke: #' + Math.floor(Math.random() * 16777215).toString(16);
            $(this).attr('style', randomColor);
            })
...
*/
      ...
      value: function _textRenderer(name, extra, textClass) {
        ...
        node += name +'\n';
		node += '<br /><span style="font-size: 9px;">' + (extra.birth !== "__ ___ ____" ? extra.birth : "") + '</span>';
        node += '</p>\n';
        return node;
      }
    }, {
    ...
    _preprocess: function _preprocess(data, opts) {
      ...
      var reconstructTree = function reconstructTree(person, parent) {
        ...
        _.forEach(person.marriages, function (marriage, index) {
          ...
          /*
          if (spouse.extra.extraTree == true) {
